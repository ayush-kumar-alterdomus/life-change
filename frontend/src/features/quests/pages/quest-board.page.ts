...
import {
  ...
  IonLabel, IonBadge, IonButton, IonIcon, IonProgressBar,
  IonFab, IonFabButton, IonRefresher, IonRefresherContent,
  IonSkeletonText, IonToast, IonText
...
@Component({
        ...
        <ng-template>
          <ion-header>
            <ion-toolbar>
              ...
              <ion-button slot="end" fill="clear" (click)="showCreateModal = false">Close</ion-button>
            </ion-toolbar>
          </ion-header>
          <ion-content class="ion-padding">
            <ion-list>
              <ion-item>
                <ion-input label="Title" labelPlacement="stacked" placeholder="e.g. Run 5km"
                  [(ngModel)]="newQuest.title" maxlength="100"></ion-input>
              </ion-item>
              <ion-item>
                <ion-textarea label="Description" labelPlacement="stacked"
                  placeholder="Optional" [(ngModel)]="newQuest.description" maxlength="500"></ion-textarea>
              </ion-item>
              <ion-item>
                <ion-select label="Difficulty" labelPlacement="stacked" [(ngModel)]="newQuest.difficulty">
              ...
              <ion-item>
                <ion-input label="XP Reward (10-300)" labelPlacement="stacked" type="number"
                  [(ngModel)]="newQuest.xpReward" min="10" max="300"></ion-input>
              </ion-item>
              <ion-item>
                <ion-select label="Stat Type" labelPlacement="stacked" [(ngModel)]="newQuest.statType">
              ...
              <ion-item>
                <ion-select label="Frequency" labelPlacement="stacked" [(ngModel)]="newQuest.frequency">
            ...
            <ion-button expand="block" (click)="submitQuest()" [disabled]="!isNewQuestValid()" class="ion-margin-top">
              Create Quest
            </ion-button>
          </ion-content>
        </ng-template>
  ...
  styles: [`
    ...
    .quest-meta { display: flex; gap: 4px; margin-top: 4px; }
    .quest-meta ion-badge { font-size: 11px; }
    .quest-end {
      display: flex;
...
export class QuestBoardPage implements OnInit {
  ...
  newQuest = { title: '', description: '', difficulty: '', xpReward: 50, statType: '', frequency: '' };
  ...
  loadQuests() {
    ...
    this.questService.getDailyQuests().subscribe({
      next: (data) => { this.dailyQuests.set(data); this.loading.set(false); },
      error: () => { this.loading.set(false); this.toast('Failed to load quests', 'danger'); },
    });
  }
  ...
  completeQuest(quest: Quest) {
    ...
    this.questService.completeQuest(quest.id).subscribe({
      ...
      error: (err) => {
        this.completing.set(false);
        this.toast(err.status === 409 ? 'Quest already completed today' : 'Failed to complete quest', 'warning');
      },
    });
  }
  ...
  refresh(event: any) {
    this.questService.getDailyQuests().subscribe({
      next: (data) => { this.dailyQuests.set(data); event.target.complete(); },
  ...
  isNewQuestValid(): boolean {
    ...
    return !!(q.title && q.difficulty && q.xpReward >= 10 && q.xpReward <= 300 && q.statType && q.frequency);
  }
  ...
  submitQuest() {
    ...
    this.questService.createQuest({
      ...
      next: () => {
        ...
        this.newQuest = { title: '', description: '', difficulty: '', xpReward: 50, statType: '', frequency: '' };
        this.loadQuests();
  ...
  getDifficultyColor(difficulty: string): string {
    const colors: Record<string, string> = { EASY: 'success', MEDIUM: 'warning', HARD: 'danger', LEGENDARY: 'tertiary' };
